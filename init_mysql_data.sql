...
MIN(l.start_dt) as '빠른 시작',
MAX(l.end_dt) as '늦은 시작'
from learn_tbl le
join contact_tbl c on le.contact_id = c.id
join lecture_tbl l on le.lecture_id = le.id
group by c.name;
...
select
l.subject,count(le.contact_id) as '학생 인원수'
from lecture_tbl l
join learn_tbl le on l.id=le.lecture_id
group by l.subject

select * from count;

// 각과목별로 최우수점수 학생 이름과 score 가 맨위로 정렬되도록 출력하세요.
create or replace view g_score as
select l.subject,c.name as '학생이름', le.score
from  learn_tbl le
join contact_tbl c on le.contact_id = c.id
join lecture_tbl l on le.lecture_id = le.id
order by l.subject, le.score desc;

select * from g_score;

// 각과목별로 최하위점수 학생 이름과 score 가 맨위로 정렬되도록 출력하세요.
create or replace view l_score as
select l.subject,c.name as '학생이름', le.score
from learn_tbl le
join contact_tbl c on le.contact_id = c.id
join lecture_tbl l on le.lecture_id = le.id
order by l.subject, le.score asc;
